...
export default function WaLayout({ children }) {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      router.replace('/login?redirect=/wa');
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#111b21]">
        <div className="flex flex-col items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-[#25D366] flex items-center justify-center animate-pulse">
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#111b21]">
        <div className="text-center space-y-4">
          ...
          <p className="text-[#8696a0] text-sm">No tienes permisos para acceder a esta sección.</p>
          <button onClick={() => window.close()} className="mt-4 px-4 py-2 bg-[#25D366] text-white rounded-xl text-sm font-semibold">
            Cerrar ventana
          </button>
        </div>
      </div>
